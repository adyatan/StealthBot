...
import stealth.modules.sql.feds_sql as sql
from stealth.modules.helper_funcs.alternate import send_message
...
def new_fed(bot: Bot, update: Update):
    ...
    if not fednam == '':
        ...
        if not x:
            message.reply_text(
                "Can't federate! Please contact @stealthSupport if the problem persist.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @stealthSupport should this problem persist!")
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @stealthSupport.")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @stealthSupport !")
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "stealth_fbanned_users.json"
                message.reply_document(
                    document=output,
                    filename="stealth_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "stealth_fbanned_users.csv"
                message.reply_document(
                    document=output,
                    filename="stealth_fbanned_users.csv",
