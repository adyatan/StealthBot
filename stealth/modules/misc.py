...
from stealth.modules.helper_funcs.extraction import extract_user
import stealth.modules.sql.users_sql as sql
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if user.id == OWNER_ID:
            text += f'\nThis person is">God</a>'
    elif user.id in DEV_USERS:
            text += f'\nThis person is">Dev</a>'
    elif user.id in SUDO_USERS:
            text += f'\nThis person is">Sudo user.</a>'
    elif user.id in SUPPORT_USERS:
            text += f'\nThis person is from">Support team</a>'
    elif user.id in WHITELIST_USERS:
            text += f'\nThis person is">Whitelisted</a>'
...
def stats(_bot: Bot, update: Update):
    ...
    r = requests.get("https://api.waa.ai/v2/links/stealth").json()
