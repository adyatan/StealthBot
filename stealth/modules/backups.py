...
import stealth.modules.sql.notes_sql as sql
from stealth import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from stealth.__main__ import DATA_IMPORT
from stealth.modules.helper_funcs.chat_status import user_admin
from stealth.modules.helper_funcs.alternate import typing_action

# from stealth.modules.rules import get_rules
import stealth.modules.sql.rules_sql as rulessql

# from stealth.modules.sql import warns_sql as warnssql
import stealth.modules.sql.blacklist_sql as blacklistsql
from stealth.modules.sql import disable_sql as disabledsql

# from stealth.modules.sql import cust_filters_sql as filtersql
# import stealth.modules.sql.welcome_sql as welcsql
import stealth.modules.sql.locks_sql as locksql
from stealth.modules.connection import connected
...
def export_data(update, context):
    ...
    f = open("Stealth{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Stealth{}.backup".format(chat_id), "rb"),
        caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Stealth-Backup` is specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Stealth{}.backup".format(chat_id))  # Cleaning file
